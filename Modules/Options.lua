        ...
        args = {
            ...
            modeSwitcher = {
                ...
                args = {
                    ...
                    desc = {
                        type = "description",
                        name = "在 Hekili 主图标右侧显示一个按钮，点击可循环切换 Hekili 的显示模式（自动/单体/AOE等）。",
                        fontSize = "medium",
                        order = 2,
                        width = "full"
                    },
                    ...
                    enabled = {
                        ...
                        get = function()
                            ...
                            local db = HekiliHelper.DB.profile
                            if not db.modeSwitcher then db.modeSwitcher = { enabled = true, offsetX = 5, offsetY = 0 } end
                            return db.modeSwitcher.enabled ~= false
                        end,
                        set = function(info, val)
                            if not HekiliHelper or not HekiliHelper.DB or not HekiliHelper.DB.profile then
                                return
                            ...
                            local db = HekiliHelper.DB.profile
                            if not db.modeSwitcher then db.modeSwitcher = { enabled = true, offsetX = 5, offsetY = 0 } end
                            db.modeSwitcher.enabled = val
                            
                            -- 如果在游戏中运行，可能需要重新加载界面或手动触发模块状态更新
                            if HekiliHelper.ModeSwitcher and HekiliHelper.ModeSwitcher.frame then
                                if val then
                                    HekiliHelper.ModeSwitcher.frame:Show()
                                else
                                    HekiliHelper.ModeSwitcher.frame:Hide()
                                end
                            end
                        end
                    },
                    ...
                    offsetX = {
                        ...
                        get = function()
                            ...
                            local db = HekiliHelper.DB.profile
                            if not db.modeSwitcher then db.modeSwitcher = { enabled = true, offsetX = 5, offsetY = 0 } end
                            return db.modeSwitcher.offsetX or 5
                        end,
                        set = function(info, val)
                            if not HekiliHelper or not HekiliHelper.DB or not HekiliHelper.DB.profile then
                                return
                            ...
                            local db = HekiliHelper.DB.profile
                            if not db.modeSwitcher then db.modeSwitcher = { enabled = true, offsetX = 5, offsetY = 0 } end
                            db.modeSwitcher.offsetX = val
                            if HekiliHelper.ModeSwitcher and HekiliHelper.ModeSwitcher.frame then
                                local parent = HekiliHelper.ModeSwitcher.frame:GetParent()
                                if parent then
                                    HekiliHelper.ModeSwitcher.frame:SetPoint("LEFT", parent, "RIGHT", db.modeSwitcher.offsetX, db.modeSwitcher.offsetY)
                                end
                            end
                        end
                    },
                    ...
                    offsetY = {
                        ...
                        get = function()
                            ...
                            local db = HekiliHelper.DB.profile
                            if not db.modeSwitcher then db.modeSwitcher = { enabled = true, offsetX = 5, offsetY = 0 } end
                            return db.modeSwitcher.offsetY or 0
                        end,
                        set = function(info, val)
                            if not HekiliHelper or not HekiliHelper.DB or not HekiliHelper.DB.profile then
                                return
